...
def get_text_messages(message):
    ...
    if message.text == 'Требования к целевой страницы':
        ...
        photo = open('stranica', 'rb')
        bot.send_message(message.from_user.id, text='Целевые страницы для рекламы электронной коммерции и финансовых услуг должны отображать достоверную информацию о компании в видимом месте, включая, помимо прочего, информацию о том, кто управляет сайтом, ее правилах, ценах, отображаемых в местной валюте, условиях и бизнес-лицензии. Чтобы гарантировать, что объявления пройдут проверку, включите эту информацию в видном месте на каждой странице/URL-адресе. В идеале эта информация должна отображаться на каждой странице сайта.', photo=photo,reply_markup=markup, parse_mode='Markdown')
